...
Weitere Authentifizierungsoptionen finden Sie unter [Azure-Identitätsclientbibliothek für Java](https://github.com/Azure/azure-sdk-for-java/tree/master/sdk/identity/azure-identity#azure-identity-client-library-for-java).
...
Erstellen Sie eine neue Datei namens *AzureApp.java* im Verzeichnis *src/main/java/com/fabrikam* des Projekts, und fügen Sie den folgenden Codeblock ein. Aktualisieren Sie die Variablen `userName` und `sshKey` mit echten Werten für Ihren Computer. Durch den Code wird in der Azure-Region „USA, Osten“ ein neuer virtueller Linux-Computer namens `testLinuxVM` in der Ressourcengruppe `sampleResourceGroup` erstellt.
...
Weitere Informationen zur Ressourcenverwaltung und Aufgabenautomatisierung mit den Azure-Verwaltungsbibliotheken für Java finden Sie in unserem Beispielcode für [virtuelle Computer](virtual-machine-samples.md), [Web-Apps](web-apps-samples.md) und [SQL-Datenbank](sql-database-samples.md).
...
Stellen Sie in [Stack Overflow](https://stackoverflow.com/questions/tagged/azure+java) Fragen an die Community. Melden Sie Fehler und Probleme im Zusammenhang mit dem Azure SDK für Java im [GitHub-Repository](https://github.com/Azure/azure-sdk-for-java).
